...
if __name__=='__main__':
    ...
    parser.add_argument('--output_data_dir', type=str, default='opt/ml/output')
    ...
    metrics_location = args.output_data_dir + '/kefico-AE-result.txt'
    ...
    logger.info(f"args.window_size: {args.window_size}")
    logger.info(f"args.latent_dim: {args.latent_dim}")
    logger.info(f"args.thres: {args.thres}")   
    ...
    # logger.info(model)
    best_model_wts = os.listdir(model_location)
    if best_model_wts:
        wts_name = best_model_wts[0]
    model_path = os.path.join(model_location, wts_name)
